...
static bool
matchWild(const char *pattern, const char *testee)
{
...
static void
loadExcludeList(fs::FS &_fs, const char *filename)
{
...
static bool
isExcluded(fs::FS &_fs, const char *filename)
{
	if (excludes == NULL)
	{
...
//  #ifdef ESP32
//  SPIFFSEditor::SPIFFSEditor(const fs::FS& fs  = SPIFFS, const String&
//  username, const String& password) #else
...
bool
SPIFFSEditor::canHandle(AsyncWebServerRequest *request)
{
...
void
SPIFFSEditor::handleRequest(AsyncWebServerRequest *request)
{
	...
	if (request->method() == HTTP_GET)
	{
		...
		{
			request->send(request->_tempFile, request->_tempFile.name(),
						  String(), request->hasParam("download"));
		}
		else
		{
	...
	else if (request->method() == HTTP_DELETE)
	{
		if (request->hasParam("path", true))
		{
			...
			request->send(
				200, "", "DELETE: " + request->getParam("path", true)->value());
		}
		else
			request->send(404);
	}
	else if (request->method() == HTTP_POST)
	{
		if (request->hasParam("data", true, true) &&
			...
			request->send(
				200, "",
				"UPLOADED: " + request->getParam("data", true, true)->value());
		else
			request->send(500);
	}
	else if (request->method() == HTTP_PUT)
...
void
SPIFFSEditor::handleUpload(AsyncWebServerRequest *request,

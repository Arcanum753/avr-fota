// Auto-generated version file for module: core_json
// Generated: 2026-03-13 22:52
// Environment: TestSolo32

#ifndef CORE_JSON_VERSION_H
#define CORE_JSON_VERSION_H

// ============================================================
// ВЕРСИЯ МОДУЛЯ core_json
// ============================================================

// Числовая версия (для сравнений)
#define CORE_JSON_VERSION 7

// Строковая версия
#define CORE_JSON_VERSION_STR "7"

// ============================================================
// ДАТА ПОСЛЕДНЕГО ИЗМЕНЕНИЯ
// ============================================================

// Дата в формате yyyy.mm.dd hh.mm (как строка)
#define CORE_JSON_COMMIT_DATE_STR "%Y.>. (HEAD -> fix-sys-wifi, origin/fix-sys-wifi) 1bac8b3a873344a5d94f2ea0181cfa956fd8f6b8:%M"

// Компоненты даты (для числовых операций)
#define CORE_JSON_COMMIT_YEAR 1970
#define CORE_JSON_COMMIT_MONTH 1
#define CORE_JSON_COMMIT_DAY 1
#define CORE_JSON_COMMIT_HOUR 0
#define CORE_JSON_COMMIT_MINUTE 0

// Полная дата в формате YYYY-MM-DD HH:MM (для отладки)
#define CORE_JSON_COMMIT_DATE "%Y->- (HEAD -> fix-sys-wifi, origin/fix-sys-wifi) 1bac8b3a873344a5d94f2ea0181cfa956fd8f6b8:%M"

// ============================================================
// ИНФОРМАЦИЯ О ГЕНЕРАЦИИ
// ============================================================

#define CORE_JSON_GENERATED_TIME "2026-03-13 22:52"
#define CORE_JSON_GENERATED_TIMESTAMP "20260313_225208"

#endif // CORE_JSON_VERSION_H

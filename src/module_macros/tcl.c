...
#if defined(TCL_TRACE)
#include <stdarg.h>
void tcl_dbg_put(const char* s);
static void tcl_dbg(const char* fmt, ...) {
  char b[240];
  va_list ap;
  va_start(ap, fmt);
...
/* Защита от зависания интерпретатора: глубина вложенности tcl_eval
   и число итераций циклов ограничены (см. конец файла и tcl_cmd_while) */
#ifndef TCL_MAX_DEPTH
#define TCL_MAX_DEPTH 32
#endif
#ifndef TCL_MAX_STEPS
#define TCL_MAX_STEPS 100000
#endif
...
static int tcl_is_end(char c) { return (c == '\n' || c == '\r' || c == ';' || c == '\0'); }
...
int tcl_next(const char* s, size_t n, const char** from, const char** to, int* q) {
  ...
  *from = s;
  ...
  /* Комментарий: '#' в начале слова - пропускаем до конца строки.
     Внутри {} и "..." символ '#' остаётся литералом (не проверяется здесь). */
  if (!*q && n > 0 && *s == '#') {
    unsigned int ci = 0;
    while (ci < n && !tcl_is_end(s[ci])) { ci++; }
    *to = s + ci;
    return TCMD;
  }
  ...
  if (*s == '[' || (!*q && *s == '{')) {
    ...
    *from = *to = s + 1;
    if (*q) { return TPART; }
    if (n < 2 || (!tcl_is_space(s[1]) && !tcl_is_end(s[1]))) { return TERROR; }
    *from = *to = s + 1;
    return TWORD;
  } else if (*s == ']' || *s == '}') {
    ...
    while (i < n && (*q || !tcl_is_space(s[i])) && !tcl_is_special(s[i], *q)) { i++; }
  }
  *to = s + i;
  if (i == n) { return TERROR; }
  if (*q) { return TPART; }
...
#define tcl_each(s, len, skiperr)                                                           \
  for (struct tcl_parser p = {NULL, NULL, (s), (s) + (len), 0, TERROR};                     \
       p.start < p.end &&                                                                   \
       (((p.token = tcl_next(p.start, p.end - p.start, &p.from, &p.to, &p.q)) != TERROR) || \
        (skiperr));                                                                         \
       p.start = p.to)
...
tcl_value_t* tcl_alloc(const char* s, size_t len) { return tcl_append_string(NULL, s, len); }

tcl_value_t* tcl_dup(tcl_value_t* v) { return tcl_alloc(tcl_string(v), tcl_length(v)); }
...
tcl_value_t* tcl_list_at(tcl_value_t* v, int index) {
  ...
  tcl_each(tcl_string(v), tcl_length(v) + 1, 0) {
    if (p.token == TWORD) {
      if (i == index) {
        if (p.from[0] == '{') { return tcl_alloc(p.from + 1, p.to - p.from - 2); }
...
tcl_value_t* tcl_list_append(tcl_value_t* v, tcl_value_t* tail) {
  if (tcl_length(v) > 0) { v = tcl_append(v, tcl_alloc(" ", 2)); }
  if (tcl_length(tail) > 0) {
    ...
    if (q) { v = tcl_append(v, tcl_alloc("{", 1)); }
...
int tcl_subst(struct tcl* tcl, const char* s, size_t len) {
  ...
  switch (s[0]) {
    ...
    default: return tcl_result(tcl, FNORMAL, tcl_alloc(s, len));
  }
}

int tcl_eval(struct tcl* tcl, const char* s, size_t len) {
  static int tcl_eval_depth = 0;
  if (++tcl_eval_depth > TCL_MAX_DEPTH) {
    tcl_eval_depth--;
    return tcl_result(tcl, FERROR, tcl_alloc("max depth exceeded", strlen("max depth exceeded")));
  }
  DBG("eval(%.*s)->\n", (int)len, s);
...
static int tcl_is_ws(char c) { return (c == ' ' || c == '\t'); }

/* Исполнение скрипта по командам — реализует модуль (macroEvalChunks) */
int macroTclEvalScript(struct tcl* t, const char* src);

/* Выполнение скрипта по командам верхнего уровня для тел if/while.
   pTcl надёжно обрабатывает несколько команд только при поочерёдном вызове
   tcl_eval(), поэтому текст режем на команды и исполняем каждую отдельно. */
static int tcl_eval_commands(struct tcl* tcl, const char* src, size_t len) {
  size_t cap = 128;
  size_t alen = 0;
  char* acc = malloc(cap);
  if (acc == NULL) { return tcl_result(tcl, FERROR, tcl_alloc("out of memory", 13)); }
  int brace = 0;
  int bracket = 0;
  int inq = 0;
  int comment = 0;
  ...
  for (i = 0; i < len; i++) {
    ...
    if (c == '#' && brace == 0 && bracket == 0 && !inq) {
      /* комментарий только в начале команды (в буфере лишь пробелы) */
      size_t j = 0;
      int wsOnly = 1;
      for (j = 0; j < alen; j++) { if (!tcl_is_ws(acc[j])) { wsOnly = 0; break; } }
      if (wsOnly) { comment = 1; continue; }
    }
    if (!inq && brace == 0 && bracket == 0 && (c == '\n' || c == ';')) {
      size_t a = 0, b = alen;
      while (a < b && tcl_is_ws(acc[a])) { a++; }
      while (b > a && tcl_is_ws(acc[b - 1])) { b--; }
      if (a < b) {
        acc[b] = '\0';
        int r = tcl_eval(tcl, acc + a, (b - a) + 1);
        if (r != FNORMAL) { free(acc); return r; }
      }
      alen = 0;
      continue;
    }
    ...
    if (alen + 2 > cap) {
      cap *= 2;
      char* tmp = realloc(acc, cap);
      if (tmp == NULL) { free(acc); return tcl_result(tcl, FERROR, tcl_alloc("out of memory", 13)); }
      acc = tmp;
    }
    acc[alen++] = c;
    acc[alen] = '\0';
...
static int tcl_user_proc(struct tcl* tcl, tcl_value_t* args, void* arg) {
  ...
  int r = macroTclEvalScript(tcl, tcl_string(body));
...
static int tcl_cmd_if(struct tcl* tcl, tcl_value_t* args, void* arg) {
  ...
  while (i < n) {
    ...
    if (tcl_int(tcl->result)) {
      r = tcl_eval_commands(tcl, tcl_string(branch), tcl_length(branch) + 1);
      tcl_free(branch);
      break;
    }
    i = i + 2;
...
static int tcl_cmd_while(struct tcl* tcl, tcl_value_t* args, void* arg) {
  ...
  int steps = 0;
  for (;;) {
    if (++steps > TCL_MAX_STEPS) {
      tcl_free(cond);
      tcl_free(loop);
      return tcl_result(tcl, FERROR, tcl_alloc("while: too many iterations", strlen("while: too many iterations")));
    }
    r = tcl_eval(tcl, tcl_string(cond), tcl_length(cond) + 1);
    if (r != FNORMAL) {
      tcl_free(cond);
    ...
    r = tcl_eval_commands(tcl, tcl_string(loop), tcl_length(loop) + 1);
    switch (r) {
      case FBREAK:
        tcl_free(cond);
        tcl_free(loop);
...
void tcl_destroy(struct tcl* tcl) {
  ...
  while (tcl->cmds) {
    ...
    tcl_free(cmd->name);
    /* arg освобождаем только для пользовательских proc (он выделен через tcl_dup).
       Для внешних команд arg указывает на контекст модуля — его не трогаем. */
    if (cmd->fn == tcl_user_proc && cmd->arg) { tcl_free((tcl_value_t*)cmd->arg); }
    free(cmd);
  }
  tcl_free(tcl->result);
...
#if !defined(TEST) && !defined(ARDUINO)
#define CHUNK 1024

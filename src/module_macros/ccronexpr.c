...
/* Встраивание в прошивку: функции времени (cron_mktime/cron_time) реализует
   модуль в cron_time_bridge.cpp на базе common/TimeLib. Определения ниже
   отключаются, чтобы не было конфликта при линковке. */
#define CRON_CUSTOM_TIME_BRIDGE
...
/* Встраивание в прошивку: функции времени реализует модуль
   (cron_time_bridge.cpp) на базе common/TimeLib — время «наивное»/локальное,
   консистентное с now(). Определения из исходной библиотеки не нужны. */
#ifndef CRON_CUSTOM_TIME_BRIDGE

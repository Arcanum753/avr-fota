...
static bool stm32f4_wait_busy(ESP_PROGSWD &prog, uint32_t timeout_ms = 5000) {
  uint32_t start = millis();
  while (prog.stm32f4_flash_busy()) {
  ...
  uint32_t err = stm32f4_flash_check_error(prog);
  return (err == 0);
}

// Включение режима программирования STM32F4 (PG бит)
...
void ESP_PROGSWD::stm32f4_erase_sector(uint8_t sector_num) {
  ...
  // Ожидаем готовность
  if (!stm32f4_wait_busy(*this, 5000)) {
  ...
  uint32_t cr = stm32f_read_register(FLASH_CR_F4);
  cr &= ~(FLASH_CR_SER_F4 | FLASH_CR_SNB_F4 | FLASH_CR_MER_F4);  // очищаем SER, SNB, MER
...
void ESP_PROGSWD::stm32f4_mass_erase() {
  ...
  uint32_t cr = stm32f_read_register(FLASH_CR_F4);
  cr &= ~(FLASH_CR_SER_F4 | FLASH_CR_SNB_F4);  // очищаем SER
  cr |= FLASH_CR_MER_F4;                        // mass erase
  stm32Fx_write_register(FLASH_CR_F4, cr, 0);
  ...
  // Ожидаем завершения
  if (!stm32f4_wait_busy(*this, 30000)) {  // mass erase может быть долгим

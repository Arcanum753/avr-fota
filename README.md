# avr-fota
Программатор AVR чипов по WiFi.

## Список базового функционала:
1. Wi-Fi клиент/точка доступа;
1. Возожность работы с одной из четырёх Wi-Fi точек доступа (ТД) с индивидуальным хранением сетевых настроек каждой ТД (DHCP/ статичный IP);
1. Веб-сервер. Веб-страницы;
1. Файловая система (ФС);
...
1. Загрузка, проверка .hex файлов. Прошивка .hex файлами AVR чипа;
1. Хранение двух версий прошивок, актуальной и предыдущей, в ФС для возможноси отката на предыдущую версию в случаей не удачной актуальной версии;

## Назначение проекта.
Проект-заготовка с минимальным базовым функционалом для VSCode + Platformio. Основа для будущих проектов. 

## Назначение прошивки.
Прошивка для микроконтроллера **ESP8266 на плате Wemos D1 Mini** предназначенная для возможности прошивать микроконтроллеры семейств **AVR** AtMega/AtTiny через веб-интерфейс по локальной сети.
...
Также https://github.com/adafruit/Standalone-Arduino-AVR-ISP-programmer/ из которого взяты некоторые фрагменты кода по работе протокола ISP программатора.
...
Для содания .hex файлов для AVR чипов требуется **Atmel Stuido** или **Microchip Stuido** версий не старше 2023 года.
...
Если ESP8266 работает в режиме КЛИЕНТА точки доступа WiFi, то веб-страницы ESP8266 доступны (по умолчанию) http://esp8266_a001.local/update.html Где **esp8266_a001** это имя и серийный номер устройства;
или http://192.168.43.65/update.html Где IP выдаётся точкой доступа. Узнать IP ESP8266 можно зайдя на свою точку доступа.
Затем на странице требуется указать путь до файла  и если MD5 хеш сумма файла посчитана верно и файл не испорчен, то разблокируется кнопка **Update es8266 firmware** и начнётся самообновление прошивки ESP8266, после чего ESP8266 перезапустится.
...
**(!) После любой манипуляции с UART микроконтроллера ESP8266 происходит его перезапуск (!)**

## Первое включение ESP8266.
Если не были прописаны настройки ТД WiFi в .json файлы, то через минуту после включения ESP8266 сама перейдёт в режим ТД и в списке WiFi сетей появится ТД c именем ESP8266_A001 (имя по умолчанию) и без пароля. Подключившись с этой ТД можно зайти на на главную страницу ESP8266 по адресу http://192.168.4.1/ или  http://esp8266_a001.local (функция **MDNS** или имя в локальной сети. Может не работать, функция экспериментальная!).

## Резервирование точек доступа.
ESP8266 при включении сканирует эфир в течении минуты и если в эфире присутсвует одна из прописанных ТД, то идёт подключение к ней. Таких ТД может быть настроено до 4х и при этом можно индивидуально для каждой прописать настройки dhcp/статический IP адрес.

## Веб-авторизация.
Для предотвращения не санкционированного допуска предусмотрена авторизация доступа по логину и паролю. Пароль запрашивается при первом обращении к любой странице ESP8266. Этот же пароль служит для доступа к ESP8266 режиме ТД.

## Список доступных страниц и их назначение.
Сверху каждой страницы есть ссылоки на другие страницы с функционалом.
1. Main. Показывает текущую информацию о данном экземпляре ESP8266 и основные настройки;
1. Device configuration. Редактирование имени устройства и серийного номера. ИмяУстрйоства_СерийныйНомер дают как имя для ESP8266 в режиме ТД так и имя для MDNS. Также позволяет редактировать сигнатуру AVR чипа, имя проекта Atmel/Microchip Studio, объём AVR памяти чипа;
1. NTP Settings. Настройка адресов NTP серверов, времени опроса, часовой пояс, летнее/зимнее время. Можно настроить три сервера. Если сервер недоступен идёт переключение на другой;
...
Atmega168P  сигнатура 0x1E940B ;
Atmega328P  сигнатура 0x1E950F ;
Размер памяти программ и сигнатуру можно узнать из даташитов к чипам.
...
1. Время сборки;
Данная метаинформация затем проверяется автоматически на ESP8266 и в случае если название проекта и сигнатура чипа не совпадают с названием проекта и сигнатурой чипа настроенными на странице Device configuration, то данный .hex файл не будет допущен к прошивке в AVR чип.
...
1. В свойствах проекта, в окно редактора макросов **Post-build event command line:** добавить строку
...
**(!)ДАННАЯ ФУНКЦИЯ ИСПОЛЬЗУЕТСЯ НА СВОЙ СТРАХ И РИСК(!)**

## Откат прошивки.
В ФС ESP8266 постоянно хранится две версии hex файла - текущая и предыдущие версии. В случае если новая версия не удовлетворяет каким-либо требованиям, то по нажатию кнопки 'Rollback version" происходит прошивка AVR чипа hex файлом предыдущей версии. При этом новая версия файла удаляется из ФС.


## Условия пользования и отказ от отвественности 
...
Если вы нашли проблему, пожалуйста уточните и перепроверьте ее формулировку, а затем пришлите issue или оформите pull request в github
...
В качестве благодарности за то что автор данного проекта дал вам идею/концепцию/проект/код - можете писать имя автора в описании к вашему продукту/проекту, это ни к чему не обязывает финансово и юридически, но автору будет приятно. **Автор: Самуил Германович Арканум.**